...
describe('OuraNotification UI Tests', () => {
  ...
  it('should open context menu and run item onClick', async () => {
    const target = document.createElement('div');
    target.id = 'ctx-target';
    document.body.appendChild(target);
    let clicked = false;
    const cleanup = Oura.contextMenu('#ctx-target', [
      {
        label: 'Action',
        onClick: () => {
          clicked = true;
    ...
    const menu = document.querySelector('.oura-context-menu');
    expect(menu?.classList.contains('oura-show')).toBe(true);
    (menu?.querySelector('.oura-dropdown-item') as HTMLButtonElement).click();
    expect(clicked).toBe(true);
    cleanup();
  });

  it('should show hover card after openDelay', () => {
    vi.useFakeTimers();
    const btn = document.createElement('button');
    document.body.appendChild(btn);
    const cleanup = Oura.hoverCard(btn, { html: '<p>HC</p>', openDelay: 100 });
    btn.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
    expect(document.querySelector('.oura-hover-card')?.classList.contains('oura-show')).toBe(false);
    vi.advanceTimersByTime(100);
    expect(document.querySelector('.oura-hover-card')?.classList.contains('oura-show')).toBe(true);
    cleanup();
    vi.useRealTimers();
  });
  ...
  it('should show tooltip after delay', () => {
    ...
    btn.id = 'tt-delay';
    document.body.appendChild(btn);
    const cleanup = Oura.tooltip('#tt-delay', { content: 'Delayed tip', delay: 80 });
    btn.dispatchEvent(new MouseEvent('mouseenter', { bubbles: true }));
